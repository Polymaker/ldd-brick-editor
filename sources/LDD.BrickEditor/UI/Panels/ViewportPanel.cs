...
using LDD.BrickEditor.Rendering.Shaders;
using LDD.Modding;
...
using LDD.BrickEditor.Rendering.Gizmos;
using LDD.BrickEditor.Rendering.UI;
...
namespace LDD.BrickEditor.UI.Panels
{
    public partial class ViewportPanel : ProjectDocumentPanel, IViewportWindow
    {
            ...
            float lightConeAngle = 50f)
        {
            ...
            var lightDist = (maxDiag * 0.5f) / (float)Math.Tan(Common.Maths.MathHelper.ToRadian(lightConeAngle / 2f));
            ...
            var powerOffset = Common.Maths.MathHelper.Map(sceneBounds.Size.Length, 0, 25f, 5f, 40f);
