...
using LDD.BrickEditor.UI.Controls;
...
namespace LDD.BrickEditor.UI.Settings
{
    public partial class LddSettingsPanel : SettingsPanelBase
    {
        ...
        private void FillLddSettings(LDDEnvironment environment)
        {
        ...
        private void UpdateLifsStatus(LDDEnvironment environment)
        {
            string tmpPath = environment.ApplicationDataPath;
            if (environment != null && environment.DirectoryExists(LddDirectory.ApplicationData)
            ...
            if (environment != null && environment.DirectoryExists(LddDirectory.ProgramFiles)
        ...
        private void LddPathTextBoxes_ValueChanged(object sender, EventArgs e)
        {
            ...
            var tmpEnv = new LDDEnvironment(PrgmFilePathTextBox.Value, AppDataPathTextBox.Value);
        ...
        {
            ExtractLif(LddLif.DB);
        }

        private void ExtractAssetsButton_Click(object sender, EventArgs e)
        {
            ExtractLif(LddLif.Assets);
        }

        private void ExtractLif(LddLif lif)
        {
            try
            {
                var tmpEnv = new LDDEnvironment(PrgmFilePathTextBox.Value, AppDataPathTextBox.Value);
        ...
        {
            var defaultEnv = LDDEnvironment.FindInstalledEnvironment();
        ...
        private bool ValidateProgramPath(ref string programDir)
        {
            ...
            string exePath = Path.Combine(programDir, LDDEnvironment.EXE_NAME);
        ...
        private bool ValidateAppDataPath(ref string appDataPath)
        {
            ...
            string dbLif = Path.Combine(appDataPath, LDDEnvironment.DB_LIF);
            string dbFolder = Path.Combine(appDataPath, Path.GetFileNameWithoutExtension(LDDEnvironment.DB_LIF));
