...
namespace LDD.BrickEditor.Models
{
    public static class BrickListCache
    {
        ...
        public static void Initialize()
        {
            ...
            try
            {
                ...
                if (Core.LDDEnvironment.Current != null)
                {
                    string oldSource = SourcePath;
                    SourcePath = string.Empty;

                    if (Core.LDDEnvironment.Current.IsLifExtracted(Core.LddLif.DB))
                    {
                        Source = CacheSource.ExtractedContent;
                        SourcePath = Core.LDDEnvironment.Current.GetAppDataSubDir("db\\Primitives");
                    }
                    else if (Core.LDDEnvironment.Current.IsLifPresent(Core.LddLif.DB))
                    {
                        Source = CacheSource.LIF;
                        SourcePath = Core.LDDEnvironment.Current.GetLifFilePath(Core.LddLif.DB);
                    }
                    else
                    {
                        Source = CacheSource.None;
        ...
        public static bool CheckIfConfigurationChanged()
        {
            ...
            if (Core.LDDEnvironment.Current != null)
            {
                ...
                if (Core.LDDEnvironment.Current.IsLifExtracted(Core.LddLif.DB))
                {
                    ...
                    newSourcePath = Core.LDDEnvironment.Current.GetAppDataSubDir("db\\Primitives");
                }
                else if (Core.LDDEnvironment.Current.IsLifPresent(Core.LddLif.DB))
                {
                    newSourceType = CacheSource.LIF;
                    newSourcePath = Core.LDDEnvironment.Current.GetLifFilePath(Core.LddLif.DB);
                }
        ...
        {
            if (FSW == null &&
                Core.LDDEnvironment.Current != null &&
                Core.LDDEnvironment.Current.IsLifExtracted(Core.LddLif.DB))
            {

                FSW = new FileSystemWatcher
                {
                    Path = Core.LDDEnvironment.Current.GetAppDataSubDir("db\\Primitives"),
