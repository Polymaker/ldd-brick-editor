...
namespace LDD.BrickEditor.Meshes
{
    public static class MeshConverter
    {
        ...
        public static MeshGeometry AssimpToLdd(Scene scene, Mesh mesh)
        {
            ...
            for (int i = 0; i < mesh.VertexCount; i++)
            {
                builder.AddVertex(new Vertex()
                {
                    ...
                    TexCoord = hasUVs ? mesh.TextureCoordinateChannels[0][i].ToLDD().Xy : Common.Simple3D.Vector2.Empty
                }, false);
            }
                ...
                if (part.Primitive.Collisions.Any())
                {
                    ...
                    foreach (var collGroup in part.Primitive.Collisions.GroupBy(x => x.CollisionType))
                    {
                        ...
                        foreach (var collision in collGroup)
                        {
                            ...
                            var meshTransform =
                                Common.Simple3D.Matrix4d.FromScale(collision.GetSize() * 2f) * collision.Transform.ToMatrix4d();

                            if (collision.CollisionType == Core.Primitives.Collisions.CollisionType.Box)
                            {
                            ...
                            //connNode.Metadata.Add("Type", new Assimp.Metadata.Entry(Assimp.MetaDataType.String, conn.Type.ToString()));
                            collNode.Transform = ((Common.Simple3D.Matrix4)meshTransform).ToAssimp();
        ...
        public static Mesh LddMeshToAssimp(Core.Files.MeshFile meshFile)
        {
        ...
        private static string GetBoneName(Core.Primitives.FlexBone flexBone)
        {
